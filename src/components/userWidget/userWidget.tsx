...
const UserWidget = () => {
  ...
  return (
    <div className="flex items-center space-x-2">
      {dataUser && dataUser.token ? (
        // Si el usuario está logueado, mostrar opciones según el rol
        <>
          {dataUser.user.role === "ADMIN" && (
            <Link href="/admin-panel">
              <button className="secondary-btn py-1 px-2">
                <i className='bx bxs-cog' style={{ fontSize: "1rem" }}></i> Admin Panel
              </button>
            </Link>
          )}
          <Link href="/profile">
            <button className="secondary-btn py-1 px-2">
              <i className='bx bxs-user' style={{ fontSize: "1rem" }}></i> Profile
            </button>
          </Link>
          <button className="secondary-btn py-1 px-2" onClick={handleLogout}>
